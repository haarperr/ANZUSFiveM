...
exports('isCarrying', isCarrying)

local function getCarrying()
...
local function stopCarrying()
  if carry.InProgress then
    ...
    TriggerServerEvent("ag_misc:carry:stop", carry.targetSrc)
    carry.targetSrc = 0
  end
end

exports('stopCarrying', stopCarrying)
...
RegisterCommand("carry", function(source, args)
  if not carry.InProgress then
    ...
    TriggerServerEvent("ag_misc:carry:sync", targetSrc)
...
RegisterNetEvent("ag_misc:carry:syncTarget", function(targetSrc)
  local targetPed = GetPlayerPed(GetPlayerFromServerId(targetSrc))
  carry.InProgress = true
  ensureAnimDict(carry.personCarried.animDict)
  AttachEntityToEntity(cache.ped, targetPed, 0, carry.personCarried.attachX, carry.personCarried.attachY, carry.personCarried.attachZ, 0.5, 0.5, 180, false, false, false, false,
    2, false)
  carry.type = "beingcarried"
end)

RegisterNetEvent("ag_misc:carry:cl_stop", function()
  carry.InProgress = false
  ClearPedSecondaryTask(cache.ped)
  DetachEntity(cache.ped, true, false)
end)
