local function smashCase(index)
  ...
  success = lib.progressCircle({
    duration = math.abs(Config.SmashBaseTime * 1000 * (1 - weaponConf)),
    label = 'Smashing case..',
    canCancel = true,
    disable = {
      car = true,
  ...
  TriggerServerEvent('ag_artgallery:server:swapCase', { index = index })
  TriggerServerEvent('ag_artgallery:server:getReward', { key = 'case', index = index })
end

function SetupCaseZone(index)
  local config = Config.Artgallery.case[index]
    ...
    options = {
      {
        ...
        canInteract = function()
          return not Config.Artgallery.case[index].busy
        end,
        onSelect = function()
          if not IsEnoughPoliceAvailable() then return end
          AlertPolice()
          TriggerServerEvent('ag_artgallery:server:setBusy', { key = 'case', index = index, bool = true })
          smashCase(index)
        end,
        distance = 2
      }
...
RegisterNetEvent('ag_artgallery:client:swapCase', function(data)
  local config = Config.Artgallery.case[data.index]
