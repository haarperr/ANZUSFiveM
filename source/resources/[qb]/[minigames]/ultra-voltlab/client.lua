...
local function openVoltLab(time, cb)
    --Reset previous data
    ...
    if time < 10 or time > 60 then
        cb(-1, 'Entered time is out of range')
        isCallbackSent = true
        return
    end
    ...
    local iconValues = shuffle(iconValues)
    ...
    if iconValueLimit > 111 or iconValueLimit <= 0 then
        cb(-1, 'Icon values are out of range in config')
        isCallbackSent = true
        return
    end
            ...
            if activeNumberNode ~= 0 and activeCircleNode ~= 0 then
                for i, k in pairs(nodePaths[tostring(activeNumberNode)][tostring(activeCircleNode)]) do
                    if activeNumberNode and activeCircleNode then
                        drawRect(k[1], k[2], k[3], k[4], circlNodeColor[activeNumberNode][1], circlNodeColor[activeNumberNode][2], circlNodeColor[activeNumberNode][3],
                            connectedNodeAlpha[activeNumberNode])
                    end
                end
            end
                ...
                for index, freezeData in pairs(confirmedNodes) do
                    for i, k in pairs(nodePaths[tostring(freezeData[1])][tostring(freezeData[2])]) do
                        if freezeData[1] and freezeData[2] then
                            drawRect(k[1], k[2], k[3], k[4], circlNodeColor[freezeData[1]][1], circlNodeColor[freezeData[1]][2], circlNodeColor[freezeData[1]][3],
                                connectedNodeAlpha[freezeData[1]])
                        end
                    end
                end
            end
