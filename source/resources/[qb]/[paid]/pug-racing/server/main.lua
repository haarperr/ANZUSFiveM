...
RegisterNetEvent("Pug:Server:RemoveItemRacing", function(item, amount)
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
  Player.Functions.RemoveItem(item, amount)
end)

RegisterServerEvent('Pug:SV:AddItemRacing', function(item, amount)
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
  Player.Functions.AddItem(item, amount, false)
end)

local function NewRaceHasBeenCreated()
  Races = {}
  local races = exports.oxmysql:executeSync('SELECT * FROM lapraces', {})
  if races[1] ~= nil then
    for k, v in pairs(races) do
...
AddEventHandler('qb-lapraces:server:CreateLapRace', function(nameofrace)
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
  local NameEntered = nameofrace
  if IsWhitelisted(Player.PlayerData.citizenid) then
    if IsNameAvailable(NameEntered) then
      print(NameEntered)
      TriggerClientEvent('QBCore:Notify', src, 'You are making a race called ' .. NameEntered, 'success')
      TriggerClientEvent('qb-lapraces:client:StartRaceEditor', src, NameEntered)
    else
      TriggerClientEvent('QBCore:Notify', src, 'There is already a race with this name.', 'error')
    end
  else
    TriggerClientEvent('QBCore:Notify', src, 'You have not been authorized to race\'s to create.', 'error')
  end
end)
...
QBCore.Functions.CreateCallback('qb-lapraces:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
  ...
  local bool = IsWhitelisted(CitizenId)
  cb(bool, IsNameAvailable(TrackName))
end)
...
QBCore.Functions.CreateCallback('qb-lapraces:server:GetTrackData', function(source, cb, RaceId)
  local result = exports.oxmysql:executeSync('SELECT * FROM characters WHERE citizenid = ?', { Races[RaceId].Creator })
...
AddEventHandler('qb-lapraces:server:JoinRace', function(RaceData, plate)
  ...
  if Player.PlayerData.metadata['alias'] == 'NO ALIAS' then
    ...
    print(AvailableRaces[AvailableKey], 'this')
...
RegisterNetEvent('Pug:Server:SetMetaData', function(meta, data)
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
