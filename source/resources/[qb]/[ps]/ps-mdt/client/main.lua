...
RegisterCommand('mdt', function()
  ...
  PlayerData = QBCore.Functions.GetPlayerData()
  if PlayerData.metadata.isdead or PlayerData.metadata.inlaststand or PlayerData.metadata.ishandcuffed or IsPauseMenuActive() or not PlayerData.job.onduty then
    return QBCore.Functions.Notify("You must be on duty to access the MDT", "error")
  end
  ...
  GetPlayerWeaponInfos(function(weaponInfos)
    if weaponInfos and #weaponInfos > 0 then
      for _, weaponInfo in ipairs(weaponInfos) do
        TriggerServerEvent('mdt:server:registerweapon', weaponInfo.serialnumber, weaponInfo.weaponurl, weaponInfo.notes, weaponInfo.owner, weaponInfo.weapClass,
