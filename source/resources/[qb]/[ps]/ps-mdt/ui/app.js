...
$(document).ready(() => {
  ...
  $('.profile-items').on('click', '.profile-item', async function () {
    ...
    if (licenses.length == 0 || licenses.length == undefined) {
      var licenseTypes = ['pilot', 'firearm', 'driver'];
      licenses = Object.entries(
        licenseTypes.reduce((licenseType, licenseValue) => ((licenseType[licenseValue] = false), licenseType), {})
      );
    }
    ...
    if (licenses.length > 0 && (PoliceJobs[playerJob] !== undefined || DojJobs[playerJob] !== undefined)) {
      ...
      if (incidents && incidents.length > 0) {
        ...
        sortedIncidents.forEach((value) => {
          let date = new Date(Number(value.time)).toLocaleDateString('en-US', timeOptions);
          incidentsHTML += `<div class="white-tag" data-id=${value.id}>
            <div style="display: flex">
              <div class="incident-number">${value.id}</div>
              <div>
                ${value.title}
                <div class="incident-timestamp">${date}</div>
              </div>
            </div>
          </div>`;
        });
  ...
  $('#gallery-upload-input').keydown(function (e) {
    if (e.keyCode === 13) {
      let title = $('#gallery-upload-input-title').val();
      ...
      if (URL !== '') {
        ...
        $('.gallery-inner-container').prepend(
          // TODO: edit
          `<div src="${URL}" title="${title}" class="white-tag gallery-img ${randomNum}">${title}</div>`
        );
  ...
  $('.manage-profile-save').click(function () {
    if (canSaveProfile == true) {
      ...
      setTimeout(() => {
        ...
        $('.gallery-inner-container') // save profile gallery
          .find('div')
          .each(function () {
  ...
  $('.manage-incidents-title-holder').on('click', '.manage-incidents-create', function () {
    ...
    $('#manage-incidents-title-input').val('Type of Incident | Location | ' + $('.date').html());
  ...
  $('.contextmenu').on('click', '.remove-image-incident', function () {
    $('.manage-incidents-evidence-holder div')
  ...
  $('.manage-incidents-tags-holder').on('keydown', '.tag-incident-input', function (e) {
    if (e.keyCode === 13) {
      $('.manage-incidents-tags-holder').prepend(
        `<div class="manage-incidents-tag tag">${$('.tag-incident-input').text()}</div>`
      );
  ...
  $('#incidents-upload-input').keydown(function (e) {
    if (e.keyCode === 13) {
      ...
      let title = $('#incidents-upload-input-title').val();
      let cid = $('.manage-profile-citizenid-input').val();
      if (URL !== '') {
        let randomNum = Math.ceil(Math.random() * 10).toString();
        $('.manage-incidents-evidence-holder').prepend(
          `<div src="${URL}" title="${title}" class="white-tag incidents-img ${randomNum}">${title}</div>`
        );
      ...
      .each(function (index) {
        if ($('.associated-incidents-user-holder').children().eq(index).data('id') == stupidasscid) {
          ...
          $('#current-charges-holder').prepend(
            `<div class="current-charges-tag" data-link="${randomNum}">${$('.associated-incidents-user-holder')
              .children()
              .eq(index)
              .html()}</div>`
          );
  ...
  $('.incidents-person-search-holder').on('click', '.incidents-person-search-item', function () {
    ...
    if (sourceElement === 'incidents-civilians') {
      $('.manage-incidents-civilians-holder').append(`<div class="tag">${$(this).data('name')}</div>`);
      ...
      $('.reports-civilians-tags-holder').append(`<div class="tag">${$(this).data('name')}</div>`);
      ...
      $('.incidents-ghost-holder').prepend(
        `
            <div class="associated-incidents-user-container" data-id="${$(this).data('cid')}">
                ...
                <div class="associated-incidents-fine-input" data-id="${$(this).data(
                  'cid'
          ...
          this
                ...
                <div class="associated-incidents-sentence-input" data-id="${$(this).data(
                  'cid'
  ...
  $('#reports-gallery-upload-input').keydown(function (e) {
    if (e.keyCode === 13) {
      ...
      let title = $('#reports-gallery-upload-title').val();
      let cid = $('.manage-profile-citizenid-input').val();
      if (URL !== '' && title !== '') {
        let randomNum = Math.ceil(Math.random() * 10).toString();
        $('.reports-gallery-inner-container').prepend(
          `<div class="white-tag reports-img" title="${title}" src="${URL}">${title}</div>`
        );
  ...
  $('.contextmenu').on('click', '.reports-remove-image', function () {
    $('.reports-gallery-inner-container div')
      .filter("[src='" + $(this).data('info') + "']")
      .remove();
  });
    ...
    if (e.keyCode === 13 && canSearchForVehicles == true) {
      ...
      if (name !== '') {
        ...
        result.forEach((value) => {
          ...
          if (value.plate_stolen) {
            plate_stolen = 'green-tag';
          }
                            ...
                            <div style="display: flex; flex-direction: column;">
                                <div class="dmv-item-title">${value.model}</div>
                                    <div class="dmv-tags">
                                        ...
                                        <div class="dmv-tag ${plate_stolen}">Plate Stolen</div>
                                    </div>
                                </div>
  ...
  $('.vehicle-information-title-holder').on('click', '.vehicle-information-save', function () {
    if (canSaveVehicle) {
      ...
      setTimeout(() => {
        ...
        let plate_stolen = false;
        let plate_stolentag = $('.vehicle-tags').find('.plate_stolen-tag');
        if (plate_stolentag.hasClass('green-tag')) {
          plate_stolen = true;
        }
  ...
  $('.contextmenu').on('click', '.mark-plate-stolen', function () {
    let tag = $('.vehicle-tags').find('.plate_stolen-tag');
    if (tag.hasClass('red-tag')) {
      tag.removeClass('red-tag').addClass('green-tag');
    }
  });

  $('.contextmenu').on('click', '.remove-plate-stolen', function () {
    let tag = $('.vehicle-tags').find('.plate_stolen-tag');
    if (tag.hasClass('green-tag')) {
      tag.removeClass('green-tag').addClass('red-tag');
    }
  ...
  $('.vehicle-tags').on('contextmenu', '.plate_stolen-tag', function (e) {
    let plate = $('.vehicle-info-plate-input').val();
    if (plate) {
      let args = [];
      if ($(this).hasClass('red-tag')) {
  ...
  $('.dispatch-items').on('contextmenu', '.dispatch-item-message', function (e) {
    ...
    var mySubString = Callsign.substring(Callsign.indexOf('(') + 1, Callsign.lastIndexOf(')'));
  ...
  function applyCustomTheme(theme) {
    document.documentElement.style.setProperty('--color-1' /* Panels */, theme.color1);
    document.documentElement.style.setProperty('--color-2' /* Clock */, theme.color2);
    document.documentElement.style.setProperty('--color-3' /* Outlines and hover */, theme.color3);
    document.documentElement.style.setProperty('--color-4' /* Button Base */, theme.color4);
  ...
  window.addEventListener('message', function (event) {
    ...
    if (eventData.type == 'show') {
      ...
      $.each(eventData.bulletin, function (index, value) {
        $('.bulletin-items-continer').prepend(`<div class="bulletin-item" data-id=${value.id} data-title=${value.title}>
      ...
      if ($('.respond-calls-container').data('id') == eventData.callid) {
        $('.respond-calls-responses').prepend(
          `<div class="respond-calls-response"> ${eventData['name']} responded "${eventData['message']}" - ${timeAgo(
      ...
      $.each(table, function (index, value) {
        $('.respond-calls-responses').prepend(
          `<div class="respond-calls-response"> ${value['name']} responded "${value['message']}" - ${timeAgo(
            Number(value.time)
          )}. </div>`
        );
      });
        ...
        $.each(value, function (i, v) {
          $(`#penal-${index}`).append(`
                    <div class="offense-item ${
                      v.color
                    }-penal-code" data-id="${index}.${i}" data-sentence="${v.months}" data-fine="${v.fine}" data-descr="${v.description}">
                    ...
                    <div style="display: flex; flex-direction: row; width: 100%; margin: auto; margin-bottom: 0vh; padding-top: 0.75vh;">
                        ...
                        <div class="offfense-item-months">${
                          v.months
                        } Months - ${typeof v.fine === 'string' ? v.fine : '$' + v.fine}</div>
      ...
      $.each(table['officersinvolved'], function (index, value) {
        $('.manage-incidents-officers-holder').append(`<div class="tag">${value}</div>`);
      });

      $('.manage-incidents-civilians-holder').empty();
      $.each(table['civsinvolved'], function (index, value) {
        $('.manage-incidents-civilians-holder').append(`<div class="tag">${value}</div>`);
      });

      $('.manage-incidents-evidence-holder').empty();
      $.each(table['evidence'], function (index, value) {
        $('.manage-incidents-evidence-holder').append(
          `<div class="white-tag incidents-img" title="${value.title}" src="${value.src}">${value.title}</div>`
        );
      });
      ...
      $.each(table['gallery'], function (index, value) {
        ...
        $('.reports-gallery-inner-container').append(
          `<div class="white-tag reports-img" title="${value.title}" src="${value.src}">${value.title}</div>`
        );
      });
      ...
      $('.vehicle-tags').append(`<div class="vehicle-tag ${plate_stolen} plate_stolen-tag">Plate Stolen</div>`);
      $('.vehicle-info-imageurl-input').val(table['image']);
      ...
      $.each(table, function (index, value) {
        $('.stafflogs-box').append(
          `<p style="margin : 0; padding-top:0.8vh;">► ${
            value.text
          } <span style="color: grey; float: right; padding-right: 1vh;">(${timeAgo(Number(value.time))})</span></p>`
        );
      });
      ...
      $('.vehicle-tags').find('.impound-tag').removeClass('green-tag').addClass('red-tag');
    }
  });
});
...
function timeShit() {
  ...
  date = localDate.toLocaleDateString('en-US', timeOptions);
  time = date.split(',')[1];
  day = date.split(',')[0];
  $('.date').html(day);
  $('.time').html(time);
...
function hideIncidentsMenu() {
  ...
  if ($('.convictions-known-container').css('display') != 'none' && !mouse_is_inside) {
    $('.convictions-known-container').fadeOut(0);
    $('.close-all').css('filter', 'none');
  }
  if ($('.incidents-known-container').css('display') != 'none' && !mouse_is_inside) {
