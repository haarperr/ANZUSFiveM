...
if Config.UseWolfknightRadar == true then
  ...
  AddEventHandler("wk:onPlateScanned", function(cam, plate, index)
    ...
    if bolo == true then
      TriggerClientEvent('QBCore:Notify', src, 'BOLO ID: ' .. boloId .. ' | Title: ' .. title .. ' | Registered Owner: ' .. vehicleOwner .. ' | Plate: ' .. plate, 'error',
        Config.WolfknightNotifyTime)
    end
    if warrant == true then
      TriggerClientEvent('QBCore:Notify', src, 'WANTED - INCIDENT ID: ' .. incidentId .. ' | Registered Owner: ' .. owner .. ' | Plate: ' .. plate, 'error',
        Config.WolfknightNotifyTime)
    end
...
RegisterNetEvent("ps-mdt:server:ClockSystem", function()
  ...
  if PlayerData.job.onduty then
    ...
    MySQL.Async.insert(
      'INSERT INTO mdt_clocking (user_id, firstname, lastname, clock_in_time) VALUES (:user_id, :firstname, :lastname, NOW()) ON DUPLICATE KEY UPDATE user_id = :user_id, firstname = :firstname, lastname = :lastname, clock_in_time = NOW()',
      {
        user_id = PlayerData.citizenid,
        firstname = firstName,
        lastname = lastName
    ...
    sendToDiscord(16711680, "Clock-Out",
      'Officer: ' ..
      firstName ..
      " " .. lastName .. '\nJob: ' .. PlayerData.job.name .. '\nRank: ' .. PlayerData.job.grade.name .. '\nStatus: Off Duty\n\nTotal time:' .. time_formatted)
  end
  ...
  if Player then
    ...
    if JobType ~= nil then
      local people = MySQL.query.await(
        "SELECT p.citizenid, p.charinfo, md.pfp, md.fingerprint FROM characters p LEFT JOIN mdt_data md on p.citizenid = md.cid WHERE LOWER(CONCAT(JSON_VALUE(p.charinfo, '$.firstname'), ' ', JSON_VALUE(p.charinfo, '$.lastname'))) LIKE :query OR LOWER(`charinfo`) LIKE :query OR LOWER(`citizenid`) LIKE :query OR LOWER(md.fingerprint) LIKE :query AND jobtype = :jobtype LIMIT 20",
...
QBCore.Functions.CreateCallback('mdt:server:GetProfileData', function(source, cb, sentId)
  ...
  if Config.PoliceJobs[JobName] or Config.DojJobs[JobName] then
    ...
    for key, value in pairs(vehicles) do
      local data = AG.GetVehicleData(value.model)
      vehicles[key].model = data.name
    end
...
RegisterNetEvent('psmdt-mugshot:server:MDTupload', function(citizenid, MugShotURLs)
  ...
  local cid = citizenid
  MySQL.Async.insert(
    'INSERT INTO mdt_data (cid, pfp, gallery, tags) VALUES (:cid, :pfp, :gallery, :tags) ON DUPLICATE KEY UPDATE cid = :cid,  pfp = :pfp, tags = :tags, gallery = :gallery', {
      cid = cid,
      pfp = MugShotURLs[1],
      tags = json.encode(tags),
  ...
  if Player then
    ...
    if JobType == 'police' or JobType == 'doj' then
      local vehicles = MySQL.query.await(
        "SELECT pv.id, pv.owner, pv.plate, pv.model, JSON_EXTRACT(pv.data, '$.properties') as mods, pv.out, pv.vin, p.charinfo FROM `character_vehicles` pv LEFT JOIN characters p ON pv.owner = p.citizenid WHERE LOWER(`plate`) LIKE :query OR LOWER(pv.`vin`) LIKE :query LIMIT 25",
        {
    ...
    if Player then
      ...
      if JobType == 'police' or JobType == 'doj' then
        local vehicle = MySQL.query.await(
          "select pv.owner, pv.plate, pv.garage, pv.id, pv.model, JSON_EXTRACT(pv.data, '$.properties') AS mods, p.charinfo from character_vehicles pv LEFT JOIN characters p ON pv.owner = p.citizenid where pv.plate = :plate LIMIT 1",
        ...
        if vehicle and vehicle[1] then
          ...
          if info then
            ...
            vehicle[1]['plate_stolen'] = info['plate_stolen']
          end


          local data = AG.GetVehicleData(vehicle[1]['model'])
          vehicle[1]['model'] = data.make .. ' ' .. data.name
...
RegisterNetEvent('mdt:server:saveVehicleInfo', function(dbid, plate, imageurl, notes, stolen, plate_stolen, code5, impoundInfo, points)
  if plate then
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player then
      if GetJobType(Player.PlayerData.job.name) == 'police' then
        ...
        if tonumber(dbid) == 0 then
          MySQL.insert(
            'INSERT INTO `mdt_vehicleinfo` (`plate`, `information`, `image`, `code5`, `stolen`, `plate_stolen`, `points`) VALUES (:plate, :information, :image, :code5, :stolen, :plate_stolen, :points)',
            {
              plate = string.gsub(plate, "^%s*(.-)%s*$", "%1"),
              information = notes,
              image = imageurl,
        ...
        elseif tonumber(dbid) > 0 then
          MySQL.update(
            "UPDATE mdt_vehicleinfo SET `information`= :information, `image`= :image, `code5`= :code5, `stolen`= :stolen, `plate_stolen` = :plate_stolen, `points`= :points WHERE `plate`= :plate LIMIT 1",
            {
              plate = string.gsub(plate, "^%s*(.-)%s*$", "%1"),
              information = notes,
              image = imageurl,
        ...
        if impoundInfo.impoundChanged then
          local vehicle = MySQL.single.await(
            "SELECT p.id, p.plate, i.vehicleid AS impoundid FROM `character_vehicles` p LEFT JOIN `mdt_impound` i ON i.vehicleid = p.id WHERE plate=:plate",
            { plate = string.gsub(plate, "^%s*(.-)%s*$", "%1") })
          if impoundInfo.impoundActive then
            ...
            if vehicle.impoundid ~= nil then
              local data = vehicle
              local result = MySQL.single.await("SELECT id, vehicle, fuel, engine, body FROM `character_vehicles` WHERE plate=:plate LIMIT 1",
                { plate = string.gsub(plate, "^%s*(.-)%s*$", "%1") })
              if result then
                local data = result
...
RegisterNetEvent('mdt:server:setCallsign', function(cid, newcallsign)
  ...
  TriggerClientEvent('mdt:client:updateCallsign', Player.PlayerData.source, newcallsign)
end)
...
RegisterNetEvent('mdt:server:saveIncident', function(id, title, information, tags, officers, civilians, evidence, associated, time)
  ...
  if Player then
    if GetJobType(Player.PlayerData.job.name) == 'police' then
      ...
      elseif id > 0 then
        MySQL.update(
          "UPDATE mdt_incidents SET title=:title, details=:details, civsinvolved=:civsinvolved, tags=:tags, officersinvolved=:officersinvolved, evidence=:evidence WHERE id=:id", {
            title = title,
            details = information,
            tags = json.encode(tags),
            officersinvolved = json.encode(officers),
  ...
  if Player then
    if GetJobType(Player.PlayerData.job.name) == 'police' then
      if sentInfo and type(sentInfo) == 'table' then
        ...
        if (plate and linkedreport and fee and time) then
          local vehicle = MySQL.query.await("SELECT id, plate FROM `character_vehicles` WHERE plate=:plate LIMIT 1", { plate = string.gsub(plate, "^%s*(.-)%s*$", "%1") })
          if vehicle and vehicle[1] then
            ...
            MySQL.insert('INSERT INTO `mdt_impound` (`vehicleid`, `linkedreport`, `fee`, `time`) VALUES (:vehicleid, :linkedreport, :fee, :time)', {
              ...
              local data = {
                ...
                vehicle = sentVehicle,
                officer = Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname,
                number = Player.PlayerData.phone_number,
                time = os.time() * 1000,
                src = src,
              }
              local vehicle = NetworkGetEntityFromNetworkId(sentVehicle)
...
RegisterNetEvent('mdt:server:removeImpound', function(plate, currentSelection)
  ...
  if Player then
    if GetJobType(Player.PlayerData.job.name) == 'police' then
      local result = MySQL.single.await("SELECT id, vehicle FROM `character_vehicles` WHERE plate=:plate LIMIT 1", { plate = string.gsub(plate, "^%s*(.-)%s*$", "%1") })
...
function GetWarrantStatus(plate)
  local result = MySQL.query.await(
    "SELECT p.plate, p.owner as citizenid, m.id FROM character_vehicles p INNER JOIN mdt_convictions m ON p.owner = m.cid WHERE m.warrant =1 AND p.plate =?",
    { plate })
  if result and result[1] then
...
function GetVehicleOwner(plate)
  local result = MySQL.query.await('SELECT plate, citizenid, id FROM character_vehicles WHERE plate = @plate', { ['@plate'] = plate })
  if result and result[1] then
    local citizenid = result[1]['citizenid']
    local Player = QBCore.Functions.GetPlayerByCitizenId(citizenid)
    local owner = Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname
...
RegisterNetEvent('QBCore:Server:PlayerLoaded', function(player)
  ...
  if not Config.AllowedJobs[PlayerData.job.name] then return end
  local Radio = 0
...
RegisterNetEvent('QBCore:Server:OnJobUpdate', function(source, newJob)
  local src = source
  if not Config.AllowedJobs[newJob.name] then return end
  local Radio = Player(src).state.radioChannel or 0
  local PlayerData = GetPlayerData(src)
  ...
  activeUnits[PlayerData.citizenid] = {
    ...
    lastName = PlayerData.charinfo.lastname:sub(1, 1):upper() .. PlayerData.charinfo.lastname:sub(2),
    radio = Radio,
    unitType = newJob.name,
    duty = newJob.onduty
  }
end)

RegisterNetEvent('QBCore:Server::')
