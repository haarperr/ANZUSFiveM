...
local function InjuriedPerson()
    ...
    local dispatchCodeName = "civdown"
    local locale = _U("persondown")

    if Player.job and Player.job.type == "leo" then
        dispatchCodeName = "officerdown"
        locale = _U("officerdown")
        return
    end
    ...
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify", {
        dispatchcodename = dispatchCodeName, -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-69",
        firstStreet = locationInfo,
        gender = gender,
        model = nil,
...
RegisterNetEvent('ps-dispatch:FleecaBankRobbery', function()
    FleecaBankRobbery()
end)
...
RegisterNetEvent('ps-dispatch:PaletoBankRobbery', function()
    PaletoBankRobbery()
end)
...
RegisterNetEvent('ps-dispatch:PacificBankRobbery', function()
    PacificBankRobbery()
end)
...
RegisterNetEvent('ps-dispatch:VangelicoRobbery', function()
    VangelicoRobbery()
end)
...
local function OfficerDown()
    ...
    TriggerServerEvent("dispatch:server:notify", {
        ...
        job = { "ambulance", "lspd" }        -- jobs that will get the alerts
    })
end
...
RegisterNetEvent('ps-dispatch:Explosion', function()
    Explosion()
end)
...
RegisterNetEvent('ps-dispatch:SuspiciousActivity', function()
    SuspiciousActivity()
end)
...
local function OfficerDownNonUrgent()
    local plyData = QBCore.Functions.GetPlayerData()
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local callsign = QBCore.Functions.GetPlayerData().metadata["callsign"]
    TriggerServerEvent("dispatch:server:notify", {
        dispatchcodename = "paniconurgent", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-13B",
        firstStreet = locationInfo,
        name = "Officer - " ..
            plyData.charinfo.firstname:sub(1, 1):upper() ..
        ...
        dispatchMessage = _U('panicnoturgent'), -- message
        job = { "lspd", "bcso" }                -- jobs that will get the alerts
    })
end
exports('OfficerDownNonUrgent', OfficerDownNonUrgent)

local function OfficerDownUrgent()
    local plyData = QBCore.Functions.GetPlayerData()
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local callsign = QBCore.Functions.GetPlayerData().metadata["callsign"]
    TriggerServerEvent("dispatch:server:notify", {
        dispatchcodename = "panicurgent", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-13",
        firstStreet = locationInfo,
        name = "Officer - " ..
            plyData.charinfo.firstname:sub(1, 1):upper() ..
        ...
        dispatchMessage = _U('panicurgent'), -- message
        job = { "lspd", "bcso" }             -- jobs that will get the alerts
    })
end
exports('OfficerDownUrgent', OfficerDownUrgent)
