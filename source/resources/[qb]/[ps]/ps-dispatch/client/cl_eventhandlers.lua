...
local function isPlayerNPC()
    local playerPed = PlayerPedId()

    for _, ped in ipairs(GetGamePool('CPed')) do
        local pedType = GetPedType(ped)

        if not IsPedAPlayer(ped) and pedType ~= 28 then
            local distance = #(GetEntityCoords(ped) - GetEntityCoords(playerPed))
            
            if distance < Config.DistanceWitnesses then
                return true
            end
        end
    end
    ...
    if vehicle ~= 0 then
        if vehicleWhitelist[GetVehicleClass(vehicle)] then
            ...
            if Config.Timer['Shooting'] == 0 and not BlacklistedWeapon(ped) and not IsPedCurrentWeaponSilenced(ped) and IsPedArmed(ped, 4) then
                if IsPedShooting(ped) and isPlayerNPC() then 
        ...
        if Config.Timer['Shooting'] == 0  and not IsPedCurrentWeaponSilenced(ped) and IsPedArmed(ped, 4) then
            if IsPedShooting(ped) and  isPlayerNPC() and not BlacklistedWeapon(ped) then
