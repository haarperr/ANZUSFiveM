...
local MinimumSpeed = Config.MinimumSpeed
local MinimumSpeedUnbuckled = Config.MinimumSpeedUnbuckled
local next = next
local nos = 0
local stress = 0
local hunger = 100
...
local AddStress = Config.AddStress
local Menu = {
    ...
    isMarkChecked = true,        -- isMapEnabledChecked
...
local function hasHarness()
    ...
    local hasHarness = exports.ag_misc:HasHarness()
    if hasHarness then
        _harness = true
    else
        _harness = false
...
RegisterNUICallback('showOutWatermark', function(data, cb)
    cb({})
    Wait(50)
    if data.checked then
        Menu.isMarkChecked = true
    else
        Menu.isMarkChecked = false
    end
    TriggerEvent("hud:client:playHudChecklistSound")
end)
...
RegisterNUICallback('updateMenuSettingsToClient', function(data, cb)
    ...
    Menu.isMarkChecked = data.isMarkChecked
    ...
    while true do
        if LocalPlayer.state.isLoggedIn then
            ...
            if IsPedInAnyVehicle(ped, false) then
                ...
                local stressSpeed = seatbeltOn and MinimumSpeed or MinimumSpeedUnbuckled
                if speed >= stressSpeed then
                    TriggerServerEvent('hud:server:GainStress', math.random(AddStress.on_fastdrive.min, AddStress.on_fastdrive.max))
                end
            end
        end
        Wait(10000)
    ...
    while true do
        if LocalPlayer.state.isLoggedIn then
            ...
            if weapon ~= `WEAPON_UNARMED` then
                local is = IsPedShooting(ped)
                if is then
                    if math.random() < config.StressChance then
                        TriggerServerEvent('hud:server:GainStress', math.random(AddStress.on_shoot.min, AddStress.on_shoot.max))
                    end
                    Wait(100)
                else
                    Wait(100)
