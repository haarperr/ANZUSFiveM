...
function UpdateAllScenes()
    scenes = {}
    MySQL.Async.fetchAll('SELECT * FROM scenes WHERE date_deletion > NOW()', {}, function(result)
        if result[1] then
...
RegisterNetEvent('qb-scenes:server:DeleteScene', function(id)
    MySQL.Async.execute('UPDATE scenes SET date_deletion = NOW() WHERE id = ?', {id}, function()
        UpdateAllScenes()
    end)
end)
