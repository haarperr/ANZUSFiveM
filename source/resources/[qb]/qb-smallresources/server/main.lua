...
RegisterNetEvent("Tackle:Server:TackleNearbyPlayers", function()
    local src = source
    local srcPed = GetPlayerPed(src)
    local tackledPlayers = {}

    -- Make targets ragdoll.
    ...
    for i = 1, #players do
        ...
        if src ~= target then
            ...
            if srcCoords and targetCoords and #(srcCoords - targetCoords) <= 2.5 then
                table.insert(tackledPlayers, target)
