    ...
    if radioMenu then
        ...
        if not firstTime then
            ...
            if PlayerData.job.type == 'leo' and PlayerData.job.onduty then
                channel = 1
            elseif PlayerData.job.name == 'ambulance' and PlayerData.job.onduty then
                channel = 9
            else
                channel = math.random(100, 450)
            end
            connecttoradio(channel)
        end

        SendNUIMessage({ type = "open", channel = RadioChannel })
    else
...
RegisterCommand('+turnonradio', function(source, args, rawCommand)
    if not hasRadio then return end
    connecttoradio(lastChannel)
end, false)

RegisterCommand('-leaveradio', function(source, args, rawCommand)
    if not hasRadio then return end
...
TriggerEvent("chat:removeSuggestion", "/+turnonradio")
TriggerEvent("chat:removeSuggestion", "/-leaveradio")

RegisterKeyMapping('+radio', 'Open Radio', 'keyboard', 'P')
RegisterKeyMapping('+turnonradio', 'Turn on Radio', 'keyboard', 'LBRACKET')
RegisterKeyMapping('-leaveradio', 'Turn off Radio', 'keyboard', 'RBRACKET')
...
RegisterNUICallback('joinRadio', function(data, cb)
    ...
    if rchannel == nil then return QBCore.Functions.Notify(Config.messages['invalid_radio'], 'error') end

    if rchannel > Config.MaxFrequency or rchannel == 0 then return QBCore.Functions.Notify(Config.messages['invalid_radio'], 'error') end

    if rchannel == RadioChannel then return QBCore.Functions.Notify(Config.messages['you_on_radio'], 'error') end

    if Config.RestrictedChannels[rchannel] == nil then return connecttoradio(rchannel) end

    if Config.RestrictedChannels[rchannel][PlayerData.job.name] and not PlayerData.job.onduty then
        return QBCore.Functions.Notify(Config.messages['restricted_channel_error'], 'error')
    end
...
RegisterNUICallback("increaseradiochannel", function(_, cb)
    ...
    local canUse = false
    if Config.RestrictedChannels[newChannel] then
...
RegisterNUICallback("decreaseradiochannel", function(_, cb)
    ...
    if newChannel >= 1 then
        if Config.RestrictedChannels[newChannel] and Config.RestrictedChannels[newChannel][PlayerData.job.name] and PlayerData.job.onduty then
            RadioChannel = newChannel
            exports["pma-voice"]:setRadioChannel(newChannel)
            QBCore.Functions.Notify(Config.messages["increase_decrease_radio_channel"] .. newChannel, "success")
            cb("ok")
...
AddStateBagChangeHandler('isCuffed', ('player:%s'):format(cache.serverId), function(_, _, value)
    if isCuffed == value then return end
...
CreateThread(function()
    while true do
        Wait(1000)
        if loggedIn and onRadio then
            if not hasRadio or PlayerData.metadata.isdead or PlayerData.metadata.inlaststand or isCuffed then
                if RadioChannel ~= 0 then
                    leaveradio()
                end
            end
...
RegisterNetEvent('qb-radio:client:JoinRadioChannel', function(channel)
