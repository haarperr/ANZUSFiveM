...
-- fuelingMode = 1 - Pump
-- fuelingMode = 2 - Can
local function startFueling(vehicle, isPump, payment)
	...
	if isPump then
		TriggerServerEvent('ox_fuel:pay', price, fuel, NetworkGetNetworkIdFromEntity(vehicle), payment)
	else
		TriggerServerEvent('ox_fuel:updateFuelCan', durability, NetworkGetNetworkIdFromEntity(vehicle), fuel)
	end
end

local function ShowMenu(vehicle, isPump)
	local Vehicle = Entity(vehicle).state
	local fuel = Vehicle.fuel or GetVehicleFuelLevel(vehicle)
    local amount = Config.priceTick * (100.0-fuel)
    local res = lib.callback.await('ox_fuel:getBankValue', 500)
    lib.registerContext({
        id = 'fuel_payment',
        title = 'Gas Station',
        options = {
            {
                title = 'Cash',
                disabled = amount > getMoneyAmount() and true or false,
                onSelect = function()
                    lib.hideContext()
                    startFueling(vehicle, isPump, 'cash')
                end
            },
            {
                title = 'Bank',
                disabled = amount > res.data and true or false,
                onSelect = function()
                    lib.hideContext()
                    startFueling(vehicle, isPump, 'bank')
                end
            }
        }
    })

    lib.showContext('fuel_payment')
end
