...
---@param price number
---@param payment 'cash' | 'bank'
---@return boolean?
local function defaultPaymentMethod(playerId, price, payment)
  ...
  elseif payment == 'bank' then
    exports.pefcl:removeBankBalance(playerId, { amount = price, message = 'Refuelled vehicle' })
    return true
  end
end
...
RegisterNetEvent('ox_fuel:pay', function(price, fuel, netid, payment)
	assert(type(price) == 'number', ('Price expected a number, received %s'):format(type(price)))

	if not payMoney(source, price, payment) then return end
...
lib.callback.register('ox_fuel:getBankValue', function(source)
    return exports.pefcl:getDefaultAccountBalance(source)
end)