...
local function GenerateCartridge(text)
	if text and text:len() == 6 then
		return text
	end

    return ('%s-%s'):format(math.random(1000,9999), text == nil and GenerateText(4) or text )
end
	...
	if item.weapon then
		...
		if metadata.registered ~= false and (metadata.ammo or item.name == 'WEAPON_STUNGUN') then
			...
			if registered then
				...
				metadata.cartridge = GenerateCartridge(metadata.cartridge)
			else
...
Item('berkeleypack', function(event, item, inventory, slot, data)
    if event == 'usedItem' then
        exports.ox_inventory:AddItem(inventory.id, 'cigarette', 10)
    elseif event == 'buying' then
        return TriggerClientEvent('ox_inventory:notify', inventory.id, {type = 'success', text = 'You bought a pack of Berkeley\'s'})
    end
end)

--- BOX AMMO ---

Item('pistol_boxammo', function(event, item, inventory, slot, data)
    if event == 'usedItem' then
        exports.ox_inventory:AddItem(inventory.id, 'ammo-pistol' , 50)
    elseif event == 'buying' then
        return TriggerClientEvent('ox_inventory:notify', inventory.id, {type = 'success', text = 'You bought a box of pistol ammo'})
    end
end)

Item('smg_boxammo', function(event, item, inventory, slot, data)
    if event == 'usedItem' then
        exports.ox_inventory:AddItem(inventory.id, 'ammo-smg' , 100)
    elseif event == 'buying' then
        return TriggerClientEvent('ox_inventory:notify', inventory.id, {type = 'success', text = 'You bought a box of smg ammo'})
    end
end)

Item('shotgun_boxammo', function(event, item, inventory, slot, data)
    if event == 'usedItem' then
        exports.ox_inventory:AddItem(inventory.id, 'ammo-shotgun' , 50)
    elseif event == 'buying' then
        return TriggerClientEvent('ox_inventory:notify', inventory.id, {type = 'success', text = 'You bought a box of shotgun shells'})
    end
end)

Item('rifle_boxammo', function(event, item, inventory, slot, data)
    if event == 'usedItem' then
        exports.ox_inventory:AddItem(inventory.id, 'ammo-rifle' , 150)
    elseif event == 'buying' then
        return TriggerClientEvent('ox_inventory:notify', inventory.id, {type = 'success', text = 'You bought a box of rifle ammo'})
    end
end)

Item('sniper_boxammo', function(event, item, inventory, slot, data)
    if event == 'usedItem' then
        exports.ox_inventory:AddItem(inventory.id, 'ammo-sniper' , 50)
    elseif event == 'buying' then
        return TriggerClientEvent('ox_inventory:notify', inventory.id, {type = 'success', text = 'You bought a box of sniper ammo'})
    end
end)

Item('beanbag_boxammo', function(event, item, inventory, slot, data)
    if event == 'usedItem' then
        exports.ox_inventory:AddItem(inventory.id, 'ammo-beanbag' , 10)
    elseif event == 'buying' then
        return TriggerClientEvent('ox_inventory:notify', inventory.id, {type = 'success', text = 'You bought a box of beanbag ammo'})
    end
end)

Item('emp_boxammo', function(event, item, inventory, slot, data)
    if event == 'usedItem' then
        exports.ox_inventory:AddItem(inventory.id, 'ammo-emp' , 5)
    elseif event == 'buying' then
        return TriggerClientEvent('ox_inventory:notify', inventory.id, {type = 'success', text = 'You bought a box of emp ammo'})
    end
end)

Item('mg_boxammo', function(event, item, inventory, slot, data)
    if event == 'usedItem' then
        exports.ox_inventory:AddItem(inventory.id, 'ammo-mg' , 150)
    elseif event == 'buying' then
        return TriggerClientEvent('ox_inventory:notify', inventory.id, {type = 'success', text = 'You bought a box of mg ammo'})
    end
end)

Item('taser_boxammo', function(event, item, inventory, slot, data)
    if event == 'usedItem' then
        exports.ox_inventory:AddItem(inventory.id, 'ammo-taser' , 3)
    elseif event == 'buying' then
        return TriggerClientEvent('ox_inventory:notify', inventory.id, {type = 'success', text = 'You bought a box of taser ammo'})
    end
end)

Item('methtable', function(event, item, inventory, slot, data)
  if event == 'usedItem' then
    exports.ag_drugs:spawnMethTable(inventory.id)
  end
end)

--- whitelist Blueprints ---

Item('bp_weapon_browning', function(event, item, inventory, slot, data)
	if event == 'usedItem' then
		exports['ag-blueprints']:GiveBlueprint(inventory.id, 'weapon_browning')
	end
  end)

Item('bp_weapon_heavypistol', function(event, item, inventory, slot, data)
	if event == 'usedItem' then
			exports['ag-blueprints']:GiveBlueprint(inventory.id, 'weapon_heavypistol')
	end
end)

Item('bp_weapon_pistol50', function(event, item, inventory, slot, data)
	if event == 'usedItem' then
			exports['ag-blueprints']:GiveBlueprint(inventory.id, 'weapon_pistol50')
	end
end)

Item('bp_weapon_dp9', function(event, item, inventory, slot, data)
	if event == 'usedItem' then
			exports['ag-blueprints']:GiveBlueprint(inventory.id, 'weapon_dp9')
	end
end)

Item('bp_weapon_appistol', function(event, item, inventory, slot, data)
	if event == 'usedItem' then
			exports['ag-blueprints']:GiveBlueprint(inventory.id, 'weapon_appistol')
	end
end)

Item('bp_weapon_microsmg2', function(event, item, inventory, slot, data)
	if event == 'usedItem' then
			exports['ag-blueprints']:GiveBlueprint(inventory.id, 'weapon_microsmg2')
	end
end)

Item('bp_weapon_minismg2', function(event, item, inventory, slot, data)
	if event == 'usedItem' then
			exports['ag-blueprints']:GiveBlueprint(inventory.id, 'weapon_minismg2')
	end
end)

Item('bp_weapon_pumpshotgun', function(event, item, inventory, slot, data)
	if event == 'usedItem' then
			exports['ag-blueprints']:GiveBlueprint(inventory.id, 'weapon_pumpshotgun')
	end
end)

Item('bp_weapon_assaultrifle', function(event, item, inventory, slot, data)
	if event == 'usedItem' then
			exports['ag-blueprints']:GiveBlueprint(inventory.id, 'weapon_assaultrifle')
	end
end)

Item('bp_weapon_assaultrifle2', function(event, item, inventory, slot, data)
	if event == 'usedItem' then
			exports['ag-blueprints']:GiveBlueprint(inventory.id, 'weapon_assaultrifle2')
	end
end)

Item('bp_weapon_compactrifle', function(event, item, inventory, slot, data)
	if event == 'usedItem' then
			exports['ag-blueprints']:GiveBlueprint(inventory.id, 'weapon_compactrifle')
	end
end)

Item('bp_weapon_militaryrifle', function(event, item, inventory, slot, data)
	if event == 'usedItem' then
			exports['ag-blueprints']:GiveBlueprint(inventory.id, 'weapon_militaryrifle')
	end
end)
-----------------------------------------------------------------------------------------------
