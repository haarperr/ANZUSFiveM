...
function server.hasLicense(inv, license)
	...
	if license == 'firearm' then
    local licenseItem = Inventory.GetSlotWithItem(inv, license..'_license')
    if not licenseItem then return false end

    if not licenseItem.metadata then
      local can = false
      for _, value in pairs(licenseItem) do
        if value.metadata.citizenid ~= player.PlayerData.citizenid or licenseItem.metadata.status ~= 'VALID' then
          can = false
        elseif player.PlayerData.metadata.licences[license] then
          can = true
          break
        end
      end
    ...
    if licenseItem.metadata.citizenid ~= player.PlayerData.citizenid then return false end
    if licenseItem.metadata.status ~= 'VALID' then return false end
	end
