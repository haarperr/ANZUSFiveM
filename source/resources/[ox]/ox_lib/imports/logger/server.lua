    ...
    if type(source) == 'number' and source > 0 then
        ...
        if not data then
            ...
            for i = 0, GetNumPlayerIdentifiers(source) - 1 do
                ...
                if not identifier:find('ip') and not identifier:find('live') and not identifier:find('xbl') then
                    num += 1
            ...
            if p then
              num += 1
              _data[num] = ('citizenid:%s'):format(p.citizenid)
            end
...
if service == 'loki' then
    ...
    function lib.logger(source, event, message, ...)
        ...
        local values = message.message and message or {message = message}
        ...
        for k,v in pairs(tagsTable) do
          if k == 'economy' then
            stream[k] = v -- Store the tags in the values table ready for logging
          else
            values[k] = v
          end
        end
