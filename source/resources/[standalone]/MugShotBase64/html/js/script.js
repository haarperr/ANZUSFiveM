async function getBase64Image(src, removeImageBackGround, callback, outputFormat) {
  ...
  if (img.complete || img.complete === undefined) {
    img.src =
      'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACEAAAAkCAIAAACIS8SLAAAAKklEQVRIie3NMQEAAAgDILV/55nBww8K0Enq2XwHDofD4XA4HA6Hw+E4Wwq6A0U+bfCEAAAAAElFTkSuQmCC';
    img.src = src;
  }
}
...
async function Convert(pMugShotTxd, removeImageBackGround, id) {
  let tempUrl =
    'https://nui-img/' + pMugShotTxd + '/' + pMugShotTxd + '?t=' + String(Math.round(new Date().getTime() / 1000));
  if (pMugShotTxd == 'none') {
...
async function removebackground(SentCanvas) {
  ...
  for (let i = 0; i < map.length; i++) {
    ...
    [newImgData[i * 4], newImgData[i * 4 + 1], newImgData[i * 4 + 2], newImgData[i * 4 + 3]] = !map[i]
      ? [255, 255, 255, 0]
      : [r, g, b, a];
  }
