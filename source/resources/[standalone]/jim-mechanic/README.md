...
- Nitrous relies on three columns in the database `character_vehicles`
...
- This checks the `character_vehicles` database for if the vehicle is player owned or not.
...
- The included `vehaddon.sql` file needs to be imported into your character_vehicles database to add the appropriate columns (traveldistance, hasnitro, noslevel)
...
local result = MySQL.Sync.fetchAll('SELECT plate FROM character_vehicles WHERE plate = ?', {plate})
if result[1] ~= nil then
    MySQL.Async.execute('UPDATE character_vehicles SET drivingdistance = ? WHERE plate = ?', {amount, plate})
end
```
