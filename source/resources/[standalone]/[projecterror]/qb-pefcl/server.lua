...
local function getCash(src)
    return inventory:GetItem(src, 'money', false, true)
end
...
local function UniqueAccounts(player)
    ...
    if Config.BusinessAccounts[PlayerJob.name] then
        ...
        if role then
            ...
            exports.pefcl:removeUserFromUniqueAccount(playerSrc, {
                source = playerSrc,
                accountIdentifier = PlayerJob.name,
                userIdentifier = citizenid,
            })
...
AddEventHandler("QBCore:Server:OnMoneyChange", function(playerSrc, moneyType, amount, action, reason)
    ...
    if action == "add" then exports.pefcl:addBankBalance(playerSrc, { amount = amount, message = reason }) end
    if action == "remove" then exports.pefcl:removeBankBalance(playerSrc, { amount = amount, message = reason }) end
    if action == "set" then exports.pefcl:setBankBalance(playerSrc, { amount = amount, message = reason }) end
end)
