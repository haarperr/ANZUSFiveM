...
export class InvoiceService {
  ...
  async payInvoice(req: Request<PayInvoiceInput>) {
    ...
    try {
      const fromAccount =
        req.data.fromAccountId != undefined
          ? await this._accountDB.getAccountById(req.data.fromAccountId, t)
          : req.data.toIdentifier != undefined
          ? await this._accountDB.getDefaultAccountByIdentifier(req.data.toIdentifier, t)
          : null;
      ...
      if (req.data.force != undefined && !req.data.force && accountBalance < amount) {
      ...
      t.commit();
      if (req.data.force) {
        emitNet(Broadcasts.ForcePaidInvoice, req.source);
      }
    } catch (err) {
      t.rollback();
      logger.error(err);
