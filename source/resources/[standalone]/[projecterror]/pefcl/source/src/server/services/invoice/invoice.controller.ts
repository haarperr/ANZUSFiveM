...
export class InvoiceController {
  ...
  @Export(ServerExports.ForcePayInvoice)
  @NetPromise(InvoiceEvents.ForcePayInvoice)
  async forcePayInvoice(req: Request<CreateInvoiceInput>, res: Response<Invoice>) {
    try {
      const data = await this._InvoiceService.createInvoice(req.data);

      await this._InvoiceService.payInvoice({
        source: req.source,
        data: { invoiceId: data.toJSON().id, force: true, toIdentifier: req.data.toIdentifier },
      });
