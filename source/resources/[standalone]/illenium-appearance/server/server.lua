...
RegisterServerEvent("illenium-appearance:server:saveAppearance", function(appearance)
  ...
  if appearance ~= nil then
    ...
    for i = 1, #appearance.props, 1 do
      local prop = appearance.props[i]
      if prop.drawable == -1 then goto skip end
      local label = nil
      ...
      exports.ox_inventory:AddItem(src, 'clothing', 1, {
        label = label,
        description = ('Use this to toggle your %s\n\nDrawable: %d\nTexture: %d'):format(label, prop.drawable, prop.texture),
        prop = prop.prop_id,
