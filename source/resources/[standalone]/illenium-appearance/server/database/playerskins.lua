...
function Database.PlayerSkins.UpdateActiveField(citizenID, active)
  MySQL.update.await("UPDATE character_skins SET active = ? WHERE citizenid = ?", { active, citizenID }) -- Make all the skins inactive / active
end

function Database.PlayerSkins.DeleteByModel(citizenID, model)
  MySQL.query.await("DELETE FROM character_skins WHERE citizenid = ? AND model = ?", { citizenID, model })
end

function Database.PlayerSkins.Add(citizenID, model, appearance, active)
  MySQL.insert.await("INSERT INTO character_skins (citizenid, model, skin, active) VALUES (?, ?, ?, ?)", { citizenID, model, appearance, active })
end

function Database.PlayerSkins.GetByCitizenID(citizenID, model)
  local query = "SELECT skin FROM character_skins WHERE citizenid = ?"
...
function Database.PlayerSkins.DeleteByCitizenID(citizenID)
  MySQL.query.await("DELETE FROM character_skins WHERE citizenid = ?", { citizenID })
end

function Database.PlayerSkins.GetAll()
  return MySQL.query.await("SELECT * FROM character_skins")
end
