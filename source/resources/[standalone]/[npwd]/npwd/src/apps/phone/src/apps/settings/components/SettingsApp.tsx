...
const CustomWallpaperPremiumLevel = 3;
...
export const SettingsApp: React.FC = () => {
  ...
  const wallpapers = config.wallpapers.map(
    MapSettingItem(settings.wallpaper, (val: SettingOption) => {
      // Check if the person has the right premium level to use the wallpaper
      fetchNui(PhoneEvents.GET_PREMIUM_STATUS).then((resp) => {
        const premiumLevel = resp.premiumLevel || 0;

        if (premiumLevel < val.premium) {
          addAlert({
            message: t('SETTINGS.MESSAGES.PREMIUM_REQUIRED', { premiumLevel: val.premium }),
            type: 'error',
          });
          return;
        }
  ...
  const frames = config.frames.map(
    MapSettingItem(settings.frame, (val: SettingOption) => {
      // Check if the person has the right premium level to use the frame
      fetchNui(PhoneEvents.GET_PREMIUM_STATUS).then((resp) => {
        const premiumLevel = resp.premiumLevel || 0;
  ...
  const customWallpaper: IContextMenuOption = {
    ...
    onClick: () => {
      fetchNui(PhoneEvents.GET_PREMIUM_STATUS).then((resp) => {
        ...
        if (premiumLevel < CustomWallpaperPremiumLevel) {
          addAlert({
            message: t('SETTINGS.MESSAGES.PREMIUM_REQUIRED', {
              premiumLevel: CustomWallpaperPremiumLevel,
            }),
            type: 'error',
          });
          return;
