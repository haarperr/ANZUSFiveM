...
exps('GetCitizenID', async (id: string) => {
  global.clientCitizenID = id;
  sendMessage('AGDETAIL', PhoneEvents.SET_CITIZEN_ID, id);
});

exps('SetLicences', async (licenses: any) => {
  sendMessage('AGDETAIL', PhoneEvents.SET_LICENCES, licenses);
});
