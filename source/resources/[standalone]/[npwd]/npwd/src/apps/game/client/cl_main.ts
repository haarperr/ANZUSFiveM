...
import { ClientUtils } from '@project-error/pe-utils/lib/client';
...
export const showPhone = async (): Promise<void> => {
  ...
  hasVPN = hasVPN?.metadata?.durability > new Date().getTime() / 1000 ?? false;
  sendMessage('PHONE', PhoneEvents.HAS_VPN, hasVPN);
...
// Premium level check
let eventCb: Function = null;
RegisterNuiCB(PhoneEvents.GET_PREMIUM_STATUS, (_, cb) => {
  eventCb = cb;
  emitNet(PhoneEvents.GET_PREMIUM_STATUS, {});
});

onNet(PhoneEvents.GET_PREMIUM_STATUS, (data: any) => {
