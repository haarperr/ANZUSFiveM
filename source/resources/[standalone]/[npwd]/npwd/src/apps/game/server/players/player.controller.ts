...
onNet(PhoneEvents.GET_PREMIUM_STATUS, () => {
  const source = getSource();

  const player = QBCore.Functions.GetPlayer(source);
  const premium = player.PlayerData.premium;

  return emitNet(PhoneEvents.GET_PREMIUM_STATUS, source, { premiumLevel: premium || 0 });
});
