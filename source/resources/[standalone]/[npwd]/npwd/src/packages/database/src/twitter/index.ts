...
export class _TwitterDB {
  ...
  async fetchAllTweets(profileId: number, currPage: number): Promise<Tweet[]> {
    ...
    SELECT ${SELECT_FIELDS},
        CASE WHEN u.premium IS NOT NULL AND u.premium >= ${verifiedPremiumLevel} THEN 1 ELSE 0 END AS verified
    FROM npwd_twitter_tweets
        ...
        LEFT OUTER JOIN users AS u ON u.userid = (
        		SELECT userid FROM characters WHERE characters.citizenid = npwd_twitter_tweets.identifier
        )
  ...
  async getTweet(profileId: number, tweetId: number): Promise<Tweet> {
    const query = `
        ...
        FROM npwd_twitter_tweets
            ...
            LEFT OUTER JOIN npwd_twitter_profiles AS retweets_profiles
                            ON retweets.identifier = retweets_profiles.identifier
            LEFT OUTER JOIN npwd_twitter_profiles ON npwd_twitter_tweets.profile_id = npwd_twitter_profiles.id
            LEFT OUTER JOIN users AS u ON u.userid = (
                SELECT userid FROM characters WHERE characters.citizenid = npwd_twitter_tweets.identifier
            )
